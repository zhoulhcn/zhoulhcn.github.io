 ...
 *
 *  基于 heatmap.js v2.0.0: http://www.patrick-wied.at/static/heatmapjs/
 */
(function(window) {
  'use strict'
  ...
  function define_CesiumHeatmap() {
    const CesiumHeatmap = {
      defaults: {
        useEntitiesIfAvailable: true, // 如果提供了查看器，是使用实体还是始终使用ImageProvider
        minCanvasSize: 700,           // 热图画布的最小大小（以像素为单位）
        maxCanvasSize: 2000,          // 热图画布的最大大小（以像素为单位）
        radiusFactor: 60,             // 如果没有给定半径，则使用数据点大小因子（高度和宽度中的较大者除以此数字得出使用的半径）
        spacingFactor: 1.5,           // 边界周围的额外空间（点半径乘以此数字得到间距）
        maxOpacity: 0.8,              // 如果热图选项对象中未给定，则使用的最大不透明度
        minOpacity: 0.1,              // 如果热图选项对象中未给定，则使用的最小不透明度
        blur: 0.85,                   // 如果没有在热图选项对象中给定，则使用模糊
        gradient: {                   // 如果热图选项对象中没有给定，则使用的渐变
          '.3': 'blue',
          '.65': 'yellow',
          '.8': 'orange',
          '.95': 'red'
     ...
     *  bb:      WGS84边界框 {north, east, south, west}
